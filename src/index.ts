...
app.use(
  ...
  (err: Error, req: Request, res: Response, next: NextFunction) => {
    if (err.name === 'UnauthorizedError') {
      res
        .status(401)
        .send({ code: 'INVALID_TOKEN', message: 'Token is invalid' })
    } else {
      next(err)
    }
  }
...
app.post('/employee', validate(employeeCreateRequest), postEmployee)
...
app.get(
  '/employee/salary-stats/by-department',
  getEmployeeSalaryStatsByDepartment
)

app.get(
  '/employee/salary-stats/by-sub-department',
  getEmployeeSalaryStatsBySubDepartment
)
